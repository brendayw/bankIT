package ar.edu.utn.frbb.tup.service.payments;
...
public class PaymentPlanService {
    ...
    public void createAndAssignPlan(User authenticatedUser, Loan loan, Integer months) {
        if (!loan.getClient().getUser().getId().equals(authenticatedUser.getId())) {
            throw new ValidacionException("No tiene permiso para crear una plan de pago para este préstamo");
        }
        PaymentPlan plan = new PaymentPlan();
        plan.setInstallments(months);
        if (loan.getMontoSolicitado() == null || loan.getMontoSolicitado() <= 0) {
            throw new ValidacionException("Monto solicitado inválido");
        }
        plan.setInterestRate(loan.getInteres() / loan.getMontoSolicitado());
